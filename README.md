...
> - **音声認識**: whisper.cpp (whisper-node)を使用しています。初回実行時にモデルが自動ダウンロードされます。
>
...
whisper-nodeの初回実行時、Whisperモデルファイルが自動的にダウンロードされます（数百MB～数GB）。
...
WHISPER_MODEL=base          # tiny, base, small, medium, largeから選択
WHISPER_MODEL_PATH=         # オプション: カスタムモデルディレクトリ
```
...
- `/sound add [keyword] [file] [volume]`: 新しい効果音を登録します。
  ...
  - `file`: 音声ファイルを添付（mp3 推奨）
  - `volume`: 音量（0〜200%、デフォルト 100）
...
- `lang`: 音声認識の言語設定（`ja`, `en`, `zh` など。Whisperの言語コードに準拠）
...
2. **whisper-nodeのモデルダウンロード**: 初回実行時、選択したモデルが自動的にダウンロードされます。ネットワーク接続を確認してください。

3. **ログを確認**: Botのコンソール出力にWhisperからのエラーメッセージが表示されていないか確認してください。

### モデルのダウンロードが遅い

初回実行時、whisper-nodeは選択したモデルをダウンロードします。ネットワーク速度によっては時間がかかることがあります。
...
より小さい`tiny`モデルを試すこともできます。

### パフォーマンスについて

whisper.cppはCPU最適化されており、GPUなしでも実行可能です。リアルタイム処理には`tiny`または`base`モデルを推奨します。
